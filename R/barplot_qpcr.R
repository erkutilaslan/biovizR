...
barplot_qpcr <- function(qpcr_data,
  ...
  ddCt_calc <- function(ddCt_data) {
    ...
    if (group3 != "") {
      ddCt_data <- dplyr::mutate(ddCt_data, avg_exp = dplyr::coalesce(
        target_avg_exp1,
        target_avg_exp2,
        control_avg_exp
      ))
  ...
  raw_to_percent <- function(raw_to_percent_data) {
    if (group3 == "" & group4 == "" & group4 == "") {
      raw_to_percent_data <- dplyr::arrange(
        raw_to_percent_data,
    ...
    if (group3 == "" && group4 == "" && group5 == "") {
      if (stat == TRUE) {
        final_plot <- ggpubr::ggbarplot(qpcr_plot_data,
          ...
          ggplot2::geom_errorbar(size = 1, ggplot2::aes(
            x = group2,
          ...
          ggpubr::stat_pvalue_manual(qpcr_plot_stat,
            label = "pvalue",
      ...
      if (stat == TRUE) {
        ...
        ggpubr::stat_pvalue_manual(stat2,
          label = "pvalue",
      ...
      if (stat == TRUE) {
        final_plot <- ggpubr::ggbarplot(qpcr_plot_data,
          ...
          ggpubr::stat_pvalue_manual(stat3,
            label = "pvalue",
      ...
      if (stat == TRUE) {
        final_plot <- ggpubr::ggbarplot(qpcr_plot_data,
          ...
          ggpubr::stat_pvalue_manual(stat4,
            label = "pvalue",
